...
| `PIXELROOT32_ENABLE_TILEMAP_PROJECTION=1` | `0` | 0 B SRAM | A separate, wholly flag-guarded `Renderer::drawTileMap` overload for `TileMap4bpp` taking a `const ProjectionSpec&` (a reference, not the earlier pointer — there is no null form), backed by one shared `drawTileMapProjectedImpl<TileT>` so the projected geometry exists once. The reference is a stack argument, not a field, so no per-instance SRAM is added; requires `PIXELROOT32_ENABLE_PROJECTION=1` (`#error` otherwise); the flag-off preprocessed translation unit is unchanged. Flash, measured on `esp32dev`/`examples/animated_tilemap` (a real `TileMap4bpp` consumer that calls only the plain overload, clean builds, flags forced through `PLATFORMIO_BUILD_FLAGS`): flags off 325,840 B, flags on 325,808 B — **-32 B**, effectively noise, because the projected overload and its impl are unreferenced by a caller that never invokes them and `--gc-sections` strips them entirely. Neither figure exercises the feature at runtime — no shipped example passes a non-null projection. `examples/iso_dungeon` is the wrong vehicle for this measurement: it never calls `drawTileMap` (`RoomRenderer.cpp:63`'s only match is a comment), so a figure measured there reflects incidental linker noise, not this feature — an earlier revision of this row did exactly that and reported **+16 B**. Before this feature's projected path was extracted into its own linker symbols, when the same logic was an inline runtime branch inside the always-linked plain overload, the same `examples/animated_tilemap` vehicle measured **+1,024 B**; that number no longer applies to the shipped shape. |
...
**The per-entity worst case is an upper bound, and real actors usually pay less.** A derived actor generally has trailing padding of its own, and the base-class growth is absorbed into it. Measured on `examples/bomberbot`, the only shipped consumer of depth sorting (native/64-bit, where the base grows by the larger 8 B):
