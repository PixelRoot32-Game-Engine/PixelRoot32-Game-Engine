...
**Cell-to-screen projection (opt-in, default `0`):** one capability in `pixelroot32::gameplay`, header-only with no `.cpp`. `include/gameplay/Projection.h` includes **nothing but `PlatformDefaults.h`** — every function is pure `int` arithmetic, so unlike `GridSpace.h` it does not even depend on `math/`. It is independent of `PIXELROOT32_ENABLE_GAMEPLAY_GRID_SPACE`; the only place the two meet is the `ProjectionSpec` overload of `interpolatedWorld()`, which lives in `GridMotion.h` and is guarded on both flags.

| Flag | Default | RAM Cost When Enabled | Subsystem Added |
|------|---------|------------------------|------------------|
| `PIXELROOT32_ENABLE_GAMEPLAY_PROJECTION=1` | `0` | 0 B SRAM | `gameplay::Projection.h` — cell-to-screen mapping for an arbitrary integer 2×2 basis (`ProjectionSpec`, `cellToScreenX/Y`, `screenToCellX/Y`, `projectionDet`, `projectionSpecIsValid`) |
