...
- **Conditions:** every presented frame. The win scales inversely with how much of the screen actually changes — large on localized-motion games (`2048`, `physics`, `sprites`, menu/UI screens), near-zero on full-screen scrollers (`midway_clone`, `legend_of_clone`).
...
6. **Dirty-cell census (gates C-4):** instrument `countPrevMarkedCells()` per frame across `2048`, `physics`, `sprites` (localized motion) vs `midway_clone`, `legend_of_clone` (full-screen scroll). The mean and p95 coverage fraction per example is the number that decides whether partial push is worth the High-difficulty effort — and for which games it should be enabled.
