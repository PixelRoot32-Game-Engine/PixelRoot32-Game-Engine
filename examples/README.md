...
> **Some demos live in their own repository.**
> `hello_world`, `bomberbot` and `room_screen` moved to
> **[pr32-demo-projects](https://github.com/PixelRoot32-Game-Engine/pr32-demo-projects)**,
> where they build against the published engine package instead of this
> checkout. New standalone demos go there; the examples kept here are the ones
> that exercise engine internals alongside engine development.
...
Demos that now live in [pr32-demo-projects](https://github.com/PixelRoot32-Game-Engine/pr32-demo-projects):

| Demo | What it demonstrates |
|------|----------------------|
| [hello_world](https://github.com/PixelRoot32-Game-Engine/pr32-demo-projects/tree/main/getting_started/hello_world) | Minimal `Scene`, `UILabel`, button input, background color cycle |
| [bomberbot](https://github.com/PixelRoot32-Game-Engine/pr32-demo-projects/tree/main/games/bomberbot) | Original **bomberman-style** game (all CC0 art): interpolated grid movement, deterministic seeded board generation, bounded chain-reaction explosions, PRNG enemy AI, **Y-axis depth sorting**, HUD + text overlays, `AudioEngine` |
| [room_screen](https://github.com/PixelRoot32-Game-Engine/pr32-demo-projects/tree/main/gameplay/room_screen) | `RoomGraph<N>` built from a Tilemap Editor room-layer export, with selectable per-pixel tile collision |

## Suggested learning order

Start with **[hello_world](https://github.com/PixelRoot32-Game-Engine/pr32-demo-projects/tree/main/getting_started/hello_world)** in the demo repository — engine init, one scene, text and input — then come back here:

1. **sprites** — graphics and colour models (single, dual and dual-inverted palette).  
2. **camera** or **metroidvania** / **animated_tilemap** — scrolling, tilemaps, caching (read **animated_tilemap** for the fullest tilemap write-up).  
3. **physics** — bodies, sensors, touch, area queries.  
4. **music-demo** / **2048** — **audio** (SFX events, or the **multi-track** reference in **music-demo**). **flappy_bird** — physics + OLED on a 72x40 logical screen, no audio subsystem.  
5. **midway_clone** — where the frame budget actually goes on an ESP32. Read it after step 2: it is the counter-example to the tilemap cache, and it shows how to measure rather than guess.
...
|---|---|---|
| Grid space / motion | `GAMEPLAY_GRID_SPACE` | [iso_dungeon](iso_dungeon/), [2048](2048/) |
...
| Room graph | `GAMEPLAY_ROOM` | [legend_of_clone](legend_of_clone/), [iso_dungeon](iso_dungeon/) |
