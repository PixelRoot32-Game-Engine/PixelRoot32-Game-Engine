...
* **Projection-Agnostic Static Layer Cache (`PIXELROOT32_ENABLE_STATIC_LAYER_SNAPSHOT`, default `0`)**: `graphics::StaticLayerSnapshot` caches static layers that **game code** draws, closing the gap `StaticTilemapLayerCache` cannot reach. That cache owns the `TileMap4bpp` it caches and redraws it when the cache goes cold, which presupposes a tilemap exists — an isometric or oblique floor has none, because `drawTileMap` assumes axis-aligned cells and cannot express a diamond. The snapshot inverts the relationship and never draws anything: the game calls `capture()` when the framebuffer holds its static layers and `restore()` afterwards, so what those layers are, and how they were drawn, is not the cache's business. With `PIXELROOT32_ENABLE_DIRTY_REGIONS` on, `restore()` repaints only the cells the previous frame's movers dirtied; without it, the whole buffer. Costs one logical framebuffer of heap per allocating scene (~57 KB at 240×240), which is why it defaults to off. Reference consumer: `examples/iso_dungeon`.
