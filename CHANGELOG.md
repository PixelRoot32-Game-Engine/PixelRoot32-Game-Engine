...
* **Projected Tilemap Draw (`PIXELROOT32_ENABLE_TILEMAP_PROJECTION`, default `0`)**: a separate, wholly flag-guarded `Renderer::drawTileMap` overload for `TileMap4bpp` taking a `const math::ProjectionSpec&` (a reference, not a pointer — there is no null form; the existing pointer-parameter overload of the same name is unaffected since this is unreleased) — the consumer for the two capabilities above. Backed by one shared `drawTileMapProjectedImpl<TileT>` so the projected geometry exists exactly once regardless of how many tile formats eventually call it; the plain `TileMap4bpp` overload is textually unchanged from before this feature existed. Each cell is placed through `cellToScreenX/Y` and anchored by `footYFor()` at `(centreX - tile.width / 2, centreY - footYFor(index))`, the same formula already shipped by hand in `examples/iso_dungeon/src/IsoDraw.h`; the cull window is padded by the tileset's largest sprite extent (one bounded scan over `tiles[]` per draw call, ~46 reads worst case against tilesets shipped in this repo); and dirty marking under a projection uses the sprite's own extent rather than the cell footprint, so a tile that overhangs its cell does not leave stale pixels behind. Requires `PIXELROOT32_ENABLE_PROJECTION=1` — building without it fails loudly with a named `#error` instead of a wall of missing-type errors. `TileMap` (1bpp) and `TileMap2bpp` do not have a projected overload yet. The projected path anchors from `TileMapGeneric<T>::tileFootY`, which the PixelRoot32 Tilemap Editor does not export today, so an editor-exported map anchors every tile at its top-left corner. Flash cost, measured on `esp32dev`/`examples/animated_tilemap` (a real `TileMap4bpp` consumer that calls only the plain, non-projected overload; clean builds, flags forced through `PLATFORMIO_BUILD_FLAGS`): flags off `325,840 B`, both flags on `325,808 B` — a **-32 B** move, effectively noise. This is expected, not a mistake: the projected overload and its shared implementation live in their own linker symbols, unreferenced by a binary that never calls the projected overload, so `--gc-sections` strips them entirely — the flash cost tracks *use*, not just the flag. A prior measurement of this same feature (before it was extracted into its own symbols, when the projected logic was an inline runtime branch inside the always-linked plain overload and therefore could not be stripped) found **+1,024 B** on the same vehicle; that number no longer applies to the shipped shape. Neither figure exercises the feature at runtime — no shipped example passes a non-null projection — so both measure "linked", not "executed". (An earlier draft of this entry cited `examples/iso_dungeon` and **+16 B**; that vehicle never calls `drawTileMap` at all — `RoomRenderer.cpp:63`'s only match is a comment — so `--gc-sections` stripped the feature entirely there too and the figure measured incidental linker noise, not this feature.)
