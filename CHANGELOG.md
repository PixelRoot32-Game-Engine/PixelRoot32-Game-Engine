...
* **Projected Tilemap Draw (`PIXELROOT32_ENABLE_TILEMAP_PROJECTION`, default `0`)**: an optional `const math::ProjectionSpec*` parameter on the `TileMap4bpp` overload of `Renderer::drawTileMap` — the consumer for the two capabilities above. With a null projection (the default) the path is byte-for-byte identical to before: the flag-off preprocessed translation unit is unchanged, not merely equivalent. With one, each cell is placed through `cellToScreenX/Y` and anchored by `footYFor()` at `(centreX - tile.width / 2, centreY - footYFor(index))`, the same formula already shipped by hand in `examples/iso_dungeon/src/IsoDraw.h`; the cull window is padded by the tileset's largest sprite extent (one bounded scan over `tiles[]` per draw call, ~46 reads worst case against tilesets shipped in this repo); and dirty marking under a projection uses the sprite's own extent rather than the cell footprint, so a tile that overhangs its cell does not leave stale pixels behind. Requires `PIXELROOT32_ENABLE_PROJECTION=1` — building without it fails loudly with a named `#error` instead of a wall of missing-type errors. The `TileMap` (1bpp) and `TileMap2bpp` overloads are untouched. Measured on `esp32dev`: `firmware.bin` **+16 B** with the flag on; the flag-off build is unaffected.
