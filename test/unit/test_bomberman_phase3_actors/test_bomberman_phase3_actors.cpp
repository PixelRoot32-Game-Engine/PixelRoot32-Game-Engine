/**
 * @file test_bomberman_phase3_actors.cpp
 * @brief Unit tests for Bomberman Phase 3 actor sprite wiring.
 */
...
void test_enemy_walk_sprite_for_at_rest(void) {
    TEST_ASSERT_EQUAL_PTR(&kEnemyBallomWalk[0], enemyWalkSpriteFor(0));
}

void test_enemy_walk_sprite_for_mid_step(void) {
    // (4/4) % 7 = 1
    TEST_ASSERT_EQUAL_PTR(&kEnemyBallomWalk[1], enemyWalkSpriteFor(4));
}

void test_enemy_walk_sprite_for_wraps_around_7(void) {
    // 28/4 = 7, 7 % 7 = 0
    TEST_ASSERT_EQUAL_PTR(&kEnemyBallomWalk[0], enemyWalkSpriteFor(28));
}

// --- Stage 3: Sprite table size tests (T3-TEST-SHAPE-01) ---

void test_kenemy_ballom_walk_has_7_frames(void) {
    TEST_ASSERT_EQUAL_INT(7, (int)(sizeof(kEnemyBallomWalk) / sizeof(Sprite4bpp)));
}
...
int main(int argc, char **argv) {
    ...
    RUN_TEST(test_kenemy_ballom_walk_has_7_frames);
