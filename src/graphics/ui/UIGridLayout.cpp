...
UIGridLayout::UIGridLayout(float x, float y, float w, float h)
    : UILayout(x, y, w, h) {
...
void UIGridLayout::calculateRows() {
    ...
    rows = static_cast<uint8_t>(std::ceil(static_cast<float>(elements.size()) / static_cast<float>(columns)));
}
...
void UIGridLayout::calculateCellDimensions() {
    ...
    float totalSpacingX = spacing * static_cast<float>(columns - 1);
    cellWidth = (static_cast<float>(width) - padding * 2.0f - totalSpacingX) / static_cast<float>(columns);
    
    // Calculate cell height: (height - padding * 2 - spacing * (rows - 1)) / rows
    float totalSpacingY = spacing * static_cast<float>(rows - 1);
    cellHeight = (static_cast<float>(height) - padding * 2.0f - totalSpacingY) / static_cast<float>(rows);
    
    // Ensure non-negative dimensions
    if (cellWidth < 0.0f) cellWidth = 0.0f;
    if (cellHeight < 0.0f) cellHeight = 0.0f;
}
...
void UIGridLayout::updateLayout() {
    ...
    for (size_t i = 0; i < elements.size(); ++i) {
        ...
        // Calculate position
        float elemX = x + padding + static_cast<float>(col) * (cellWidth + spacing);
        float elemY = y + padding + static_cast<float>(row) * (cellHeight + spacing);
        
        // Center element within cell if it's smaller than the cell
        float elemXCentered = elemX;
        float elemYCentered = elemY;
        
        if (static_cast<float>(elem->width) < cellWidth) {
            elemXCentered = elemX + (cellWidth - static_cast<float>(elem->width)) / 2.0f;
        }
        if (static_cast<float>(elem->height) < cellHeight) {
            elemYCentered = elemY + (cellHeight - static_cast<float>(elem->height)) / 2.0f;
        }
        ...
        float elemLeft = elemXCentered;
        float elemRight = elemXCentered + static_cast<float>(elem->width);
        float elemTop = elemYCentered;
        float elemBottom = elemYCentered + static_cast<float>(elem->height);
